...
const valueModules: CourseModule[] = [
  {
    id: 'v1',
    title: 'AXIS Foundations',
    duration: '15m',
    thumbnail: 'https://images.unsplash.com/photo-1550745679-325348393049?auto=format&fit=crop&q=80&w=800',
    videoUrl: 'https://www.youtube.com/embed/dQw4w9WgXcQ',
    description: 'Master the AXIS workflow. How to go from zero ideas to a platform-ready pulse in minutes using Gemini AI.',
    ...
    <section id="academy" className="py-32 bg-white overflow-hidden relative">
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="mb-24 text-center">
          ...
          <p className="text-violet-500 text-[11px] font-black uppercase tracking-[0.5em] mb-8 leading-loose">AXIS CREATOR HUB: Creativity Powered in One</p>
          <div className="w-24 h-2 bg-gradient-to-r from-violet-500 to-indigo-500 mx-auto rounded-full"></div>
        </div>

        <div className="grid lg:grid-cols-12 gap-16 items-start">
          <div className="lg:col-span-8">
            <div className="aspect-video bg-slate-900 rounded-[4rem] overflow-hidden border-[12px] border-slate-50 shadow-2xl">
              <iframe className="w-full h-full" src={activeMod.videoUrl} title={activeMod.title} frameBorder="0" allowFullScreen></iframe>
            </div>
            <div className="mt-12">
              <h3 className="text-4xl font-black mb-4 italic serif uppercase tracking-tighter leading-none">{activeMod.title}</h3>
          ...
          <div className="lg:col-span-4 space-y-4">
            {valueModules.map(mod => (
              <button 
                ...
                className={`w-full flex items-center gap-6 p-6 rounded-[2.5rem] transition-all text-left ${activeMod.id === mod.id ? 'bg-violet-50 ring-2 ring-violet-200 shadow-lg' : 'hover:bg-slate-50 opacity-50 hover:opacity-100'}`}
              >
                ...
                <div>
                  ...
                  <h5 className="text-lg font-black italic serif text-slate-900 leading-tight uppercase tracking-tight">{mod.title}</h5>
                </div>
              </button>
            ))}
            
            <div className="mt-12 p-10 bg-gradient-to-br from-violet-600 to-indigo-600 rounded-[3.5rem] text-center text-white shadow-2xl">
              ...
              <button className="w-full py-5 bg-white text-slate-900 font-black text-[11px] uppercase tracking-widest rounded-2xl hover:scale-105 transition-transform shadow-xl">Unlock Secret Pulses</button>
            </div>
          </div>
        </div>
      </div>
