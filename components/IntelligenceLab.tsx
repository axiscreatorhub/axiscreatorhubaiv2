  ...
  const handleGroundingSearch = async () => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: `FACELESS MARKET INTELLIGENCE: Analyze ${query} with world-class accuracy. Provide trending triggers and strategic signals for digital influencers.`,
  ...
  const handleMapsResearch = async () => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: `FACELESS GEO-INTEL: Research ${query} local creator opportunities and trending regional narratives.`,
    ...
    <section id="intelligence" className="py-48 bg-[#020617] text-white relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="max-w-4xl mb-32">
          <div className="inline-flex items-center gap-3 px-5 py-2 mb-8 rounded-full bg-white/5 border border-white/10 text-[11px] font-black uppercase tracking-[0.5em] text-indigo-400">
          ...
          <p className="text-slate-400 text-2xl leading-relaxed font-light max-w-3xl">
            Leverage Faceless Digital Life's proprietary grounding engine to identify global market shifts and regional trends with neural precision.
          </p>
        </div>
            ...
            <div className="bg-white/5 border border-white/10 rounded-[4rem] p-12 backdrop-blur-3xl shadow-2xl">
              ...
              <div className="relative">
                <input 
                  ...
                  className="w-full bg-black/20 border border-white/10 rounded-[2rem] px-10 py-8 pr-24 text-sm font-medium focus:ring-4 focus:ring-indigo-500/20 outline-none transition-all placeholder:text-slate-700"
                />
                <button 
                  onClick={handleGroundingSearch}
                  className="absolute right-4 top-1/2 -translate-y-1/2 w-16 h-16 bg-indigo-500 text-white rounded-2xl flex items-center justify-center hover:scale-105 transition-transform shadow-2xl shadow-indigo-500/30"
                >
          ...
          <div className="bg-black/60 rounded-[5rem] border border-white/10 p-16 min-h-[500px] flex flex-col relative overflow-hidden shadow-2xl">
            {isAnalyzing ? (
              <div className="flex-grow flex flex-col items-center justify-center text-center">
                <div className="w-16 h-16 border-4 border-indigo-500 border-t-transparent rounded-full animate-spin mb-10 shadow-[0_0_30px_rgba(99,102,241,0.3)]"></div>
              ...
              <div className="flex-grow animate-fadeIn overflow-y-auto space-y-10 custom-scrollbar pr-4">
                 <h4 className="text-[12px] font-black text-indigo-400 uppercase tracking-[0.6em] mb-8 border-b border-white/5 pb-4">Intelligence Manifesto</h4>
                 ...
                 {urls.length > 0 && (
                   <div className="pt-12 border-t border-white/10">
                     ...
                     <div className="space-y-4">
                       {urls.map((u, i) => u.web && (
                         <a key={i} href={u.web.uri} target="_blank" className="flex items-center justify-between p-6 bg-white/5 rounded-2xl text-[11px] hover:text-indigo-400 transition-all border border-transparent hover:border-indigo-500/30 group">
            ...
            <div className="absolute top-0 right-0 w-48 h-48 bg-indigo-500/5 rounded-full -mr-24 -mt-24 blur-[80px]"></div>
          </div>
        </div>
      </div>
    </section>
