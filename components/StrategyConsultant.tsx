...
const StrategyConsultant: React.FC = () => {
    ...
    if (!chatRef.current) {
      ...
      chatRef.current = ai.chats.create({
        ...
        config: {
          ...
          systemInstruction: "You are the Lead Strategic Consultant at AXIS | The Creator Hub. You provide world-class digital marketing and content strategy advice. You equip influencers with technical toolsets for brand architecture, viral content manifestation, and automated growth. Your tone is elite, professional, and results-oriented. AXIS is the benchmark for modern creator intelligence."
        }
      });
    }
  };
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      console.error(e);
      setMessages(prev => [...prev, { role: 'model', text: "AXIS Node Sync Interrupted. Please re-engage." }]);
    } finally {
      setIsThinking(false);
  ...
  return (
    <>
      ...
      >
        <div className="absolute -top-1 -right-1 w-5 h-5 bg-violet-500 rounded-full border-2 border-slate-900 animate-pulse"></div>
          ...
          <div className="w-full max-w-lg h-[90vh] md:h-[750px] bg-white rounded-t-[3.5rem] md:rounded-[3.5rem] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.3)] border border-slate-100 flex flex-col pointer-events-auto animate-panelIn overflow-hidden">
            <div className="p-8 bg-slate-900 text-white flex justify-between items-center relative overflow-hidden shrink-0">
              <div className="relative z-10">
                <h4 className="text-xl font-black outfit italic uppercase tracking-tight">The Strategy Room</h4>
                <div className="flex items-center gap-2 mt-1">
                  <span className={`w-2 h-2 rounded-full ${isStreaming || isThinking ? 'bg-orange-500 animate-pulse' : 'bg-emerald-500'}`}></span>
                  <span className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-400">AXIS STRATEGY OS ACTIVE</span>
                </div>
              </div>
            ...
            <div ref={scrollRef} className="flex-grow overflow-y-auto p-8 space-y-8 scroll-smooth custom-scrollbar bg-[#FDFDFD]">
              {messages.length === 0 && (
                <div className="h-full flex flex-col items-center justify-center text-center px-8 animate-fadeIn">
                  ...
                  <h5 className="text-2xl font-black outfit italic mb-3 uppercase tracking-tighter">Strategic Inquiry?</h5>
                  <p className="text-xs text-slate-400 font-bold uppercase tracking-[0.2em] leading-loose max-w-xs">Manifest your digital growth with the AXIS reasoning engine.</p>
                </div>
              )}
              {messages.map((m, i) => (
                <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'} animate-messageIn`}>
                  <div className={`group relative max-w-[88%] p-6 rounded-[2.5rem] text-sm leading-relaxed shadow-sm transition-all ${m.role === 'user' ? 'bg-slate-900 text-white rounded-tr-none' : 'bg-white text-slate-800 rounded-tl-none border border-slate-100'}`}>
                    ...
                    <div className={`absolute -bottom-6 text-[8px] font-black uppercase tracking-[0.2em] text-slate-300 ${m.role === 'user' ? 'right-4' : 'left-4'}`}>
                      {m.role === 'user' ? 'Creator' : 'AXIS Lead Strategist'}
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="p-8 pb-12 md:pb-8 bg-white border-t border-slate-50 shrink-0">
              <div className="relative group">
                <input 
                  ...
                  placeholder="Ask the AXIS engine..."
                  className="w-full bg-slate-50 border border-slate-100 rounded-[2rem] px-8 py-6 pr-16 text-sm font-medium outline-none focus:ring-1 focus:ring-violet-500 transition-all"
                />
                <button 
                  onClick={handleSend}
